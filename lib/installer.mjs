...
export function runBundledRuntimeDoctor({
  ...
  const result = runner(pythonBin, [wrapper, "doctor"], { cwd: pluginRoot, env });
  const parsed = parseRuntimeDoctorOutput(result.stdout || "");
  const ok = result.ok && parsed.compatible && parsed.source === "bundled";
  let detail = "";
  if (ok) {
    const summary = parsed.payload.contractSummary || {};
    detail = `bundled ${parsed.compatibility}; ${summary.commandCount || 0} commands; ${wrapper}`;
  } else if (!result.ok) {
