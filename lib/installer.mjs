...
export const PLUGIN_NAME = "swarm-discussion";
...
export function pluginCacheRoot({ codexHome = defaultCodexHome(), version = null } = {}) {
  const base = path.join(codexHome, "plugins", "cache", MARKETPLACE_NAME, PLUGIN_NAME);
  return version ? path.join(base, version) : base;
}

export function listPluginCacheVersions(codexHome = defaultCodexHome()) {
  const base = pluginCacheRoot({ codexHome });
  if (!existsSync(base)) {
    return [];
  }
...
export function resolveInstalledPluginRoot({ pluginStatus, codexHome = defaultCodexHome() } = {}) {
  const version = pluginStatus?.version || null;
  const marketplaceRoot = pluginStatus?.path || null;
  const cacheVersions = listPluginCacheVersions(codexHome);
  const expectedCacheRoot = version ? pluginCacheRoot({ codexHome, version }) : null;
  const exactCacheExists = Boolean(expectedCacheRoot && existsSync(expectedCacheRoot));
  const latestCacheVersion = cacheVersions.at(-1) || null;
  const latestCacheRoot = latestCacheVersion ? pluginCacheRoot({ codexHome, version: latestCacheVersion }) : null;
  const pluginRoot = exactCacheExists ? expectedCacheRoot : marketplaceRoot;
  return {
    pluginRoot,
    rootKind: exactCacheExists ? "cache" : marketplaceRoot ? "marketplace" : null,
    marketplaceRoot,
    expectedCacheRoot,
    exactCacheExists,
    cacheVersions,
    latestCacheVersion,
    latestCacheRoot,
    staleCacheVersions: version ? cacheVersions.filter((entry) => entry !== version) : cacheVersions,
    cacheVersionMismatch: Boolean(version && cacheVersions.length > 0 && !exactCacheExists)
  };
}
...
export function resolveInstalledPluginAgentSource({
  codexBin = "codex",
  cwd = process.cwd(),
  codexHome = defaultCodexHome()
} = {}) {
  ...
  const install = resolveInstalledPluginRoot({ pluginStatus: plugin.status, codexHome });
  if (plugin.status.version && !install.exactCacheExists) {
    throw new InstallerError(`Installed plugin cache for ${PLUGIN_ID} ${plugin.status.version} was not found.`, {
      expectedCacheRoot: install.expectedCacheRoot,
      cacheVersions: install.cacheVersions,
      marketplaceRoot: install.marketplaceRoot
    });
  }
  const sourceFile = pluginAgentPath(install.pluginRoot);
  const source = validateAgentSource(sourceFile);
  if (!source.ok) {
      ...
      checks.push(
        createCheck(
          ...
          plugin.status.version
            ? `version ${plugin.status.version}; listed path ${plugin.status.path}`
            : plugin.status.line
        )
      );
      if (plugin.status.version && install.exactCacheExists) {
        checks.push(createCheck(OK, "installed plugin cache root", install.expectedCacheRoot));
      } else if (plugin.status.version) {
        checks.push(
          createCheck(
            FAIL,
            "installed plugin cache root missing",
            `expected ${install.expectedCacheRoot}; cache versions: ${install.cacheVersions.join(", ") || "none"}`
          )
        );
      } else {
        checks.push(createCheck(WARN, "installed plugin cache root unknown", "plugin list did not report a version"));
      }
      if (install.staleCacheVersions.length > 0) {
        checks.push(
          createCheck(
            WARN,
            "stale plugin cache versions present",
            `${install.staleCacheVersions.join(", ")}; active ${plugin.status.version || "unknown"}`
          )
        );
      }
      if (install.exactCacheExists || !plugin.status.version) {
        sourceFile = pluginAgentPath(install.pluginRoot);
        pluginRoot = install.pluginRoot;
        const source = validateAgentSource(sourceFile);
        sourceOk = source.ok;
        checks.push(
        ...
        checks.push(createCheck(FAIL, "plugin swarm-expert template is unavailable", "installed plugin cache is missing"));
        checks.push(createCheck(FAIL, "plugin bundled runtime is unavailable", "installed plugin cache is missing"));
      }
    }
  }
